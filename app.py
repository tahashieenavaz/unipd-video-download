...
def main():
    if not os.path.exists(f'./{downloadsFolder}'): os.mkdir(downloadsFolder)
...
def findLatestId():
    ...
    while True:
        try: download(urlFor(id), f'./{downloadsFolder}/{id}.ts')
        except: break
    ...
    while True:
        ...
        try: download(urlFor(id), fileAddress) 
        except: pass
        if os.path.exists(fileAddress): break
    return id

def mergeFiles():
    parts = glob(f"./{downloadsFolder}/*.ts")
    parts.sort(key=lambda x: int(replace(x, ['.', '/', 'downloads', 'ts'], '')))
...
def downloadRest(biggest: int):
    ...
    for i in remaining:
        print(i)
        threads.append(Thread(target=download, args=(urlFor(i), f"./downloads/{i}.ts",)))
